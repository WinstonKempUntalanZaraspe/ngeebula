...
def _activity_route_and_closure(
    ...
    * when a Live closure reaches a data-defined interchange hub/shared sector,
    ...
    extended_locations = _locations_for_sector_rows(
        closure_rows, start_line, start_bound
    )
    if matched_nature == "live":
        closure = set(extended_locations)
        closure |= {_swap_bound(loc) for loc in list(closure)}
    else:
        closure = set(route)
        closure |= {
            loc for loc in extended_locations if loc.startswith("SEC:")
        }
    if matched_nature == "live":
    ...
    *,
    ...
    for i, a in enumerate(activity_ids):
        for b in activity_ids[i + 1 :]:
            a_in_b = route[a] & closure[b]
            b_in_a = route[b] & closure[a]
            directional_conflict = a_in_b | b_in_a
            if not directional_conflict:
                continue
            shared_worksite = route[a] & route[b]
            pair = {activity_access_type[a], activity_access_type[b]}
            compatible_types = (
                activity_access_type[a] == "C"
                and activity_access_type[b] == "C"
            ) or pair == {"PC", "C"}
            can_share = (
                compatible_types
                and bool(shared_worksite)
                and directional_conflict <= shared_worksite
            )
            if can_share:
                pair_coshare_exemptions += 1
                same_contract_type = (
                    activity_contract[a] == activity_contract[b]
                    and _clean(activities[a].get("activity_type"))
                    == _clean(activities[b].get("activity_type"))
                )
                for week in weeks:
                    ...
                    for night in physical_nights:
                        model.Add(
                            physical[a, week, night]
                            == physical[b, week, night]
                        ).OnlyEnforceIf(both)
                    if same_contract_type:
                        cap = _as_int(
                            projects[activity_contract[a]].get(
                                "number_of_maximum_access_per_week"
                            ),
                            f"{activity_contract[a]}.number_of_maximum_access_per_week",
                        )
                        for access_night in range(1, cap + 1):
                            model.Add(
                                local_access[a, week, access_night]
                                == local_access[b, week, access_night]
                            ).OnlyEnforceIf(both)
                continue
            pair_conflicts += 1
            for week in weeks:
