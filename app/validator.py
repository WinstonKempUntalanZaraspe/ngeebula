...
IMPORTANT caveat (unchanged from the original script): the closure check reads
``co_share_group`` as *the night*. Two activities that share a
``(location, week, co_share_group)`` anywhere in a week are co-sharing partners
and are exempt from each other's closures entirely (rule 5); otherwise an
activity may not sit inside another activity's closure on the same night.
The organisers' own sample submission uses several group labels per
activity-week, so it fails this reading on 23 closure pairs. That means our
checker is STRICTER than the official validator: a clean run here is
